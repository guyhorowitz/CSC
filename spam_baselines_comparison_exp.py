...
from model import cross_entropy_loss_for_h, hinge_loss, MLP, HelperModel, TrickyFeatureModel
from records import get_chosen_model_and_multiple_rounds_stats
from spam_params import params
...
def run(cost_scale, seed, path):
    ...
    total_dim = 15
    x1_features = [1, 8, 5]  # [13, 14]
    x2_features = [2, 3]  # [1, 2, 3]
    ...
    erm_chosen_model_stats = erm_trainer.collect_trainer_results(m_h_star, delta, full_data, x1_dim).calc_stats("ERM")
    ...
    serm_r_res = serm_r_trainer.collect_trainer_results_using_only_x2(m_h_star, delta, full_data, x1_dim)
    ...
    # create dist object from the samples that not included in the partial clean data
    s_dist = SequentialDistribution(x1_dim, X_U_train[n_clean_train_samples:])
    cserm_exp_small_trainer = IterativeCausalStrategicTrainer(f_fac, h_fac, m_h_star, delta, s_dist, clean_data, x1_dim, x2_dim, n_samples_per_round, hinge_loss,
    ...
    # create dist object from the samples that not included in the partial clean data
    s_dist = SequentialDistribution(x1_dim, X_U_train[n_clean_train_samples:])
    cserm_exp_large_trainer = IterativeCausalStrategicTrainer(f_fac, h_fac, m_h_star, delta, s_dist, clean_data, x1_dim, x2_dim, n_samples_per_round, hinge_loss,
                                                              cross_entropy_loss_for_h,
                                                              exploration_params=exploration_params, should_estimate_density=True, pca=None, cost_scale=cost_scale)
    ...
    cserm_h_star_chosen_model_stats = cserm_h_star_trainer.collect_trainer_results(m_h_star, delta, full_data, x1_dim).calc_stats("CSERM_h_star")

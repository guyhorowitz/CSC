...
from model import cross_entropy_loss_for_h, hinge_loss, MLP, TrickyFeatureModel, HelperModel
...
def run(n_flips, seed, path):
    results_file = f"spam_sensitivity_exp_v2_n_flips={n_flips}_seed={seed}"
    ...
    for perm, assumed_x1_dim in sampled_permutations:
        ...
        cserm_trainer = IterativeCausalStrategicTrainer(f_fac, h_fac, m_h_star, delta, s_dist, clean_data, assumed_x1_dim, assumed_x2_dim, n_samples_per_round, hinge_loss,
        ...
        for f in cserm_trainer.f_list:
            ...
            val_acc = test_for_sensitivity(f, m_h_star, delta, s_X_val, U_val)
            test_acc = test_for_sensitivity(f, m_h_star, delta, s_X_test, U_test)
        ...
        delta.cls = f
        temp_trainer = CausalStrategicTrainer(f, h, delta, train_dl=None, X_val=None, Y_val=None, X_test=None, x1_dim=assumed_x1_dim, opt=None, loss=None,
